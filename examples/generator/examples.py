...
import numpy as np
import os
from topologylayer.functional.utils_dionysus import top_batch_cost, cost_function
from topologylayer.functional.rips_dionysus import Diagramlayer as DiagramlayerRips
from topologylayer.functional.levelset_dionysus import (
    Diagramlayer as DiagramlayerToplevel,
)
...
def trainToplevel():
    ...
    generator.load_state_dict(
        torch.load("./generator-32000.pt", map_location="cpu")
    )  # './images_post_top/model-1330.pt', map_location="cpu"))
    ...
    z = torch.autograd.Variable(
        torch.Tensor(np.random.normal(0, 1, (28, 100))), requires_grad=True
    )
    with torch.no_grad():
