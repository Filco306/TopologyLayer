...
def plot_dgms_top(dgms):
    ...
    for i in range(len(dgms)):
        dgmpts = dgms[i]
        dgmpts = np.delete(
            dgmpts, np.where((dgmpts == (-np.inf, -np.inf)).all(axis=1)), axis=0
        )
        if len(dgmpts) > 0:
...
def save_simp_complex(
    ...
    np.savetxt(
        path + "filtration_value_{}.txt".format(name),
        np.array([filt_value]),
        delimiter=",",
    )
...
def plot_diagram2(plot_data, title="Dimension 0"):
    ...
    if len(plot_data) > 0:  # in case their is only one inf and nothing else
        ...
        if num_infs > 0:
            min_value = np.min(
                [min_value, np.min(infs[:, 0])]
            )  # include so that we can see inf
