        ...
        for i in range(F.shape[0]):
            if len(F[i]) == 4:
                c = d.closure(
                    [
                        d.Simplex(
                            [int(F[i][0]), int(F[i][1]), int(F[i][2]), int(F[i][3])],
                            0.0,
                        )
                    ],
                    3,
                )  # np.array(0).astype(DTYPE))],2)
            elif len(F[i]) == 3:
                c = d.closure(
                    [d.Simplex([int(F[i][0]), int(F[i][1]), int(F[i][2])], 0.0)], 3
                )
            for j in c:
                f.append(j)
        return f
    ...
    def forward(ctx, x, F):
        ...
        for i in range(0, len(dgm)):
            if len(dgm[i]) > 0:
                ...
                corr_function_values[i][
                    : np.min([corr_f_values.shape[0], num_dgm_pts])
                ] = corr_f_values[: np.min([corr_f_values.shape[0], num_dgm_pts])]
                vertices_indices[i][: np.min([len(dgmi), num_dgm_pts])] = dgmi[
                    : np.min([len(dgmi), num_dgm_pts])
                ]
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(len(list_of_unique_indices)):
            ...
            if index > -1:
                ...
                grad_input[int(index)] = torch.sum(
                    grad_intermediate[index_into_grad_output]
                )
