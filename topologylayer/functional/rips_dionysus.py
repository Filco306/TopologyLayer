...
class Diagramlayer(Function):
    ...
    def forward(ctx, x, saturation=None, maxdim=1, verbose=False):
        ...
        if saturation is None:
            ...
            print(
                "==== WARNING: NO SATURATION VALUE GIVEN, {}".format(SATURATION_VALUE)
            )
        else:
        ...
        for dim in range(maxdim + 1):
            if len(dgms[dim]) > 0:
                ...
                li = np.min([num_dgm_pts, len(dgm)])
                arg_sort = np.argsort(np.abs(dgm[:, 1] - dgm[:, 0]))[::-1]
                dgms_inds[dim][:li] = dgm[arg_sort[:li], 2:6]
                dgms_values[dim][:li] = dgm[arg_sort[:li], 0:2]
        ...
        ctx.save_for_backward(
            x, torch.tensor(dgms_inds).type(dtype), output, torch.tensor(verbose)
        )
        if verbose:
            print("*** dgm done", time.time() - start_time)
        return output
    ...
    def backward(ctx, grad_output):
        ...
        output[
            output == np.inf
        ] = -np.inf  # death_value infinite doesn't correspond to a simplex
        output[output > -np.inf] = 1  # actual values that map to simplices
        output[
            output == -np.inf
        ] = 0  # float('NaN') # 0 # dont affect the gradient, since they dont exist, didn't have matches, just because we want to keep matrix structure
        ...
        grad_intermediate = (
            output * grad_output.detach().numpy()
        )  # Not necessary? (dgms, dgm_pts, 2)
        ...
        for i in range(len(list_of_unique_indices)):
            index = int(
                list_of_unique_indices[i]
            )  # index into input, get all that responds to a point-index
            ...
            if index > -1:
                ...
                index_into_dgms_inds_partners[-1, :] = np.remainder(
                    index_into_dgms_inds[-1, :] + 1, 2
                )
                intermediate = (
                    pts_of_inds[list(index_into_dgms_inds)]
                    - pts_of_inds[list(index_into_dgms_inds_partners)]
                )  # - dgms_inds_to_points[np.remainder(np.array(index_into_dgms_inds)+1, 2)]
                ...
                grad_output_and_intermediate = (
                    intermediate.transpose()
                    * grad_intermediate[list(inds_into_grad_output)]
                ).transpose()
                update = np.sum(
                    grad_output_and_intermediate.reshape([-1, input.shape[1]]), axis=0
                )
                grad_input[int(index)] = torch.tensor(update).type(dtype)
        if verbose:
            print("*** dgm back done", time.time() - start_time)
