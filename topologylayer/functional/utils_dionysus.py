...
def top_batch_cost(gen_imgs, diagramlayer, filtration):
    ...
    costs = torch.stack(
        [
            top_cost(x_i.view(-1), diagramlayer, filtration)
            for i, x_i in enumerate(torch.unbind(gen_imgs, dim=axis), 0)
        ],
        dim=axis,
    )
...
def maxPolys(dgm1):
    b, d = (
        dgm1[:, np.arange(0, dgm1.shape[1] - 1, 2)],
        dgm1[:, np.arange(1, dgm1.shape[1], 2)],
    )
...
def top_features_01(x, diagramlayer, filtration):
    ...
    a1, a2, a3, a4 = (
        torch.sum(sorted[1:2]),
        torch.sum(sorted[1:3]),
        torch.sum(sorted[1:4]),
        torch.sum(sorted[1:10]),
    )
    """ dim 1 """
...
def top_batch_features(input, diagramlayer, filtration, dim=1):
    ...
    feats = torch.stack(
        [
            top_features(x_i.view(-1), diagramlayer, filtration, dim)
            for i, x_i in enumerate(torch.unbind(input, dim=axis), 0)
        ],
        dim=axis,
    )
    # avg = torch.mean(costs.view(-1))
