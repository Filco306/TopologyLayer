...
class Levelset1dsuper(unittest.TestCase):
    def test(self):
        ...
        for alg in ["hom", "hom2", "cohom"]:
            ...
            dgms, issub = layer(y)
            self.assertEqual(issub, False, "Expected superlevel set layer. alg=" + alg)
            self.assertEqual(
                torch.all(
                    torch.eq(
            ...
            self.assertEqual(y.grad[1].item(), 1.0, "unexpected gradient. alg=" + alg)
...
class Levelset1dsub(unittest.TestCase):
    def test(self):
        ...
        for alg in ["hom", "hom2", "cohom"]:
            ...
            dgms, issub = layer(y)
            self.assertEqual(issub, True, "Expected sublevel set layer. alg=" + alg)
            self.assertEqual(
                torch.all(
                    torch.eq(remove_zero_bars(dgms[0]), torch.tensor([[0.0, np.inf]]))
                ),
                True,
                "unexpected barcode. alg=" + alg,
            )
...
class Levelset2dsuper(unittest.TestCase):
    def test(self):
        ...
        for alg in ["hom", "hom2", "cohom"]:
            ...
            x = torch.tensor(
                [[2, 1, 1], [1, 0.5, 1], [1, 1, 1]], dtype=torch.float
            ).requires_grad_(True)
            ...
            self.assertEqual(
                torch.all(
                    torch.eq(remove_zero_bars(dgms[0]), torch.tensor([[2.0, -np.inf]]))
                ),
                True,
            ...
            self.assertEqual(
                torch.all(
                    torch.eq(remove_zero_bars(dgms[1]), torch.tensor([[1.0, 0.5]]))
                ),
                True,
            ...
            self.assertEqual(
                x.grad[1, 1].item(), -1.0, "unexpected gradient. alg=" + alg
            )
